...
typedef struct {
	ElemType * data;//储存数据的地址
	int length;//储存多少数据
}seqlist;

//动态内存分配地址初始化
seqlist* initlist() {
	seqlist* L = (seqlist*)malloc(sizeof(seqlist));//为顺序表开辟地址，顺序表储存数据的地址和有多少这样的数据
...
int main() {
	...
	//声明一个顺序表并初始化,返回链表地址给list
	seqlist * list = initlist();
	printf("初始化成功，目前长度占用%d\n", list->length);
	printf("目前开辟储存数据的地址为：%zu\n", list->data);
	printf("目前开辟顺序表的地址为：%zu\n", list);
